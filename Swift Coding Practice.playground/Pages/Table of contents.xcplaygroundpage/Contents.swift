/*:
# 歡迎使用Swift程式設計學習與練習
<<Playground互動學習版>>
____________________
# 目錄
****

### Swift程式設計基本練習

- [*Basic*](Basic)												基本練習題
- [*Array*](Array)												陣列
- [*Matrix*](Matrix)												矩陣
- [*String*](String)												字串
- [*Callback*](Callback)											回呼
- [*Recursive*](Recursive)											遞迴
- [*Algorithm*](Algorithm)											演算法
- [*Objective Programming*](Objective%20Programming)								物件導向
- [*Cheryls-Birthday*](Cheryls-Birthday)										生日猜謎

### Swift程式設計 -- 人工智慧

- [*Simulated Annealing*](Simulated%20Annealing)															模擬退火演算法
- [*Artificial neural network*](Artificial%20neural%20network)														神經網絡介紹
- [*Perceptron*](Perceptron)																	單層感知器(Perceptron)
- [*Multilayer Perceptron*](Multilayer%20Perceptron)															多層感知器(Multilayer perceptron)
- [*Restricted Boltzmann machine*](Restricted%20Boltzmann%20machine%20-%20RBM)													受限玻爾茲曼機
- [*Hidden Layer*](Hidden%20Layer)																隱藏層
- [*Multilayer Perceptron - MLP*](Multilayer%20Perceptron%20-%20MLP)													多層感知器(Multilayer perceptron - MLP)
- [*Deep belief network*](Deep%20belief%20network%20-%20DBN)															深度置信網路
- [*LogisticRegression*](LogisticRegression)															邏輯迴歸
- [*Convolutional neural network - CDBN*](Convolutional%20neural%20network%20-%20CDBN)											卷積神經網絡
- [*Conditional Restricted Boltzmann Machines - CRBM*](Conditional%20Restricted%20Boltzmann%20Machines%20-%20CRBM)				條件受限玻爾茲曼機
- [*Graph Traversal*](Graph%20Traversal)																圖形搜尋簡介
- [*Path Finding*](Path%20Finding)																老鼠走迷宮
- [*Puzzle Search*](Puzzle%20Search)																拼圖問題
- [*Computer chess*](Computer%20chess)																電腦下棋
- [*Gomoku*](Gomoku)																	五子棋

### 程式碼測試頁
- [*Code Testion Page*](Code%20Testion%20Page)										測試區

****
[Next](@next)
*/

